...
try:
    df = pd.read_csv('consolidacao.csv')
...
if texto_busca:
    if 'Número do processo' in df.columns:
        ...
        if not df_filtrado.empty:
            # Extrair informações
            data_chegada = df_filtrado['Data de chegada'].values[0]
            nucleo = df_filtrado['Núcleo'].values[0]
            posicao_geral = df_filtrado['Posição geral'].values[0]
            prioridade_legal = df_filtrado['Prioridade legal'].values[0]
            posicao_prioridade = df_filtrado['Posição prioridade'].values[0] if 'Posição prioridade' in df_filtrado.columns else "N/A"

            # Calcular a média de dias no núcleo
            media_dias_nucleo = calcular_media_dias(df_filtrado, 'Data de remessa', 'Data de conclusão')
